...
loadInitialStateExplicit cfg sym genTest interestingTests allOpcodes pseudoOpcodes targetOpcodes iorels = do
  let load dir = F.loadFormulasFromFiles sym (mkFormulaFilename dir) allOpcodes Nothing
  ...
  pseudoSet <- F.loadFormulasFromFiles sym (mkFormulaFilename (pseudoSetDir cfg)) pseudoOpcodes Nothing
