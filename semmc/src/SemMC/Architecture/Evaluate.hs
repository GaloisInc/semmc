...
evaluateInstruction bak semMap inst initialState =
    case inst of
        I.Instruction opc operands ->
            case MapF.lookup opc semMap of
                ...
                Just paramFormula -> do
                    (_, formula) <- F.instantiateFormula bak paramFormula operands
                    Right <$> evaluateFormula bak formula initialState
...
evaluateFormula :: forall t st fs bak arch .
...
evaluateFormula bak formula initialState =
                ...
                valueToCrucibleElt v =
                    case v of
                        ...
                        AV.ValueBV (wordValue :: W.W n) ->
                            ...
                            in SI.bvLit sym (W.rep wordValue) (BV.mkBV (W.rep wordValue) w)
            substitutions <- traverseFC bindMatchingLocation assignment0'

            newFormDefs <- traverseF
                             (\e -> S.evalBoundVars sym e vars substitutions)
                             (F.formDefs formula)
