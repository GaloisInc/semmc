...
base bitSize = runSem $ do
  ...
  defineOpcode "ADD4" $ do
    (rT, rA, rB) <- xoform3
    defLoc rT (bvadd (Param rA) (Param rB))
  defineOpcode "SUBF" $ do
    (rT, rA, rB) <- xoform3
    defLoc rT (bvsub (Param rB) (Param rA))
  defineOpcode "NEG" $ do
    rT <- param "rT" gprc naturalBV
    rA <- param "rA" gprc naturalBV
    input rA
    defLoc rT (bvadd (bvnot (Param rA)) (naturalLitBV 0x1))
  defineOpcode "MULLW" $ do
    (rT, rA, rB) <- xoform3
    let lhs = sext (lowBits 32 (Param rA))
    let rhs = sext (lowBits 32 (Param rB))
    defLoc rT (bvmul lhs rhs)
  defineOpcode "MULHW" $ do
    (rT, rA, rB) <- xoform3
    let lhs = sext (highBits 32 (Param rA))
    let rhs = sext (highBits 32 (Param rB))
    defLoc rT (bvmul lhs rhs)
  when (bitSize == Size64) $ do
    -- Not valid in 32 bit mode
    defineOpcode "MULLD" $ do
      (rT, rA, rB) <- xoform3
      let prod = bvmul (sext' 128 (Param rA)) (sext' 128 (Param rB))
  ...
  defineOpcode "XOR" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvxor (Param rS) (Param rB))
  defineOpcode "OR" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvor (Param rS) (Param rB))
  defineOpcode "AND" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvand (Param rS) (Param rB))
  defineOpcode "NAND" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvnot (bvand (Param rS) (Param rB)))
  defineOpcode "NOR" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvnot (bvor (Param rS) (Param rB)))
  defineOpcode "EQV" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvnot (bvxor (Param rS) (Param rB)))
  defineOpcode "ANDC" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvand (Param rS) (bvnot (Param rB)))
  defineOpcode "ORC" $ do
    (rA, rS, rB) <- xform3
    defLoc rA (bvor (Param rS) (bvnot (Param rB)))
  defineOpcode "SLW" $ do
    comment "Shift Left Word (X-form)"
    (rA, rS, rB) <- xform3
    let n = zext' 32 (lowBits 5 (Param rB))
    let w = lowBits 32 (Param rS)
  ...
  when (bitSize == Size64) $ do
    defineOpcode "SLD" $ do
      ...
      (rA, rS, rB) <- xform3
      let n = zext (lowBits 6 (Param rB))
      defLoc rA  (bvshl (Param rS) n)
    defineOpcode "SRD" $ do
      comment "Shift Right Doubleword (X-form)"
      (rA, rS, rB) <- xform3
      let n = zext (lowBits64 6 (Param rB))
      defLoc rA (bvlshr (Param rS) n)
  defineOpcode "ADDI" $ do
    ...
    (rT, rA, si) <- dform
    let lhs = ite (isR0 (Param rA)) (naturalLitBV 0x0) (Param rA)
    defLoc rT (bvadd lhs (sext (Param si)))
  ...
  defineOpcode "CMPWI" $ do
    ...
    inputLiteral cr
    let ximm = sext (Param imm)
    let lowreg = lowBits 32 (Param rA)
    let newCR = cmpImm bvslt bvsgt (Param fld) ximm (sext lowreg)
    defLoc cr newCR
  ...
  defineOpcode "CMPLW" $ do
    ...
    input rB
    inputLiteral cr
    let lowa = lowBits 32 (Param rA)
    let lowb = lowBits 32 (Param rB)
    let newCR = cmpImm bvslt bvsgt (Param fld) (zext lowa) (zext lowb)
    defLoc cr newCR
  ...
  defineOpcode "CMPLWI" $ do
    ...
    let ximm = zext (Param imm)
    let lowreg = lowBits 32 (Param rA)
    let newCR = cmpImm bvult bvugt (Param fld) ximm (zext lowreg)
    defLoc cr newCR
  ...
  defineOpcode "CMPLW" $ do
    ...
    let lowa = lowBits 32 (Param rA)
    let lowb = lowBits 32 (Param rB)
    let newCR = cmpImm bvult bvugt (Param fld) (zext lowa) (zext lowb)
    defLoc cr newCR
  return ()
...
pseudo bitSize = runSem $ do
  ...
  defineOpcode "ExtractByteGPR" $ do
    ...
    input source
    input n
    let shiftAmount = bvshl (zext (Param n)) (naturalLitBV 0x3)
  ...
  defineOpcode "SetSignedCR0" $ do
    ...
    let newCR = cmpImm bvslt bvsgt (LitBV 3 0x0) ximm (Param rA)
    defLoc cr newCR
  return ()
