...
baseOpcodes = map fst $(STH.attachSemantics (\(Some x) -> [ show x <.> "sem", show x <.> "stub.sem" ]) allOpcodes ["data/64/base"])
...
allSemantics = $(STH.attachSemantics (\(Some x) -> [ show x <.> "sem", show x <.> "stub.sem" ]) allOpcodes [ "data/64/base"
                                                                                          , "data/64/manual"
                                                                                          , "data/64/learned"
                                                                                          ])
